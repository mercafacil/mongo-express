...
// logger.token('formatted-url', (req, res) => {
//   try {
//     const url = new URL(req.protocol + '://' + req.get('host') + req.originalUrl);
//     const pathname = url.pathname;
//     const queryParams = new URLSearchParams(url.search);
//     const projectionParam = queryParams.get('projection');
//     const queryParam = queryParams.get('query');

//     let formattedQuery = '';
//     if (queryParam) {
//       try {
//         const parsedQuery = JSON.parse(decodeURIComponent(queryParam));
//         formattedQuery = `Query: ${JSON.stringify(parsedQuery)}`;
//       } catch (error) {
//         formattedQuery = `Query: (Erro ao parsear) ${decodeURIComponent(queryParam)}`;
//       }
//     }
...
//     if (projectionParam) {
//       try {
//         const parsedProjection = JSON.parse(decodeURIComponent(projectionParam));
//         formattedProjection = `Projection: ${JSON.stringify(parsedProjection)}`;
//       } catch (error) {
//         formattedProjection = `Projection: (Erro ao parsear) ${decodeURIComponent(projectionParam)}`;
//       }
//     }

//     let formattedUrl = pathname;
//     if (formattedQuery) {
//       formattedUrl += ` - ${formattedQuery}`;
//     }
//     if (formattedProjection) {
//       formattedUrl += ` - ${formattedProjection}`;
//     }

//     return formattedUrl;
//   } catch (error) {
//     return req.originalUrl; // Retorna a URL original em caso de erro
//   }
// });
...
logger.token('formatted-url', (req, res) => {
  try {
    ...
    if (queryParam) {
      try {
        const parsedQuery = JSON.parse(decodeURIComponent(queryParam));
        formattedQuery = `Query: ${JSON.stringify(parsedQuery)}`; // Remoção do terceiro argumento para compactar
      } catch (error) {
        formattedQuery = `Query: (Erro ao parsear) ${decodeURIComponent(queryParam)}`;
      }
    }
    ...
    if (projectionParam) {
      try {
        const parsedProjection = JSON.parse(decodeURIComponent(projectionParam));
        formattedProjection = `Projection: ${JSON.stringify(parsedProjection)}`; // Remoção do terceiro argumento para compactar
      } catch (error) {
        formattedProjection = `Projection: (Erro ao parsear) ${decodeURIComponent(projectionParam)}`;
      }
    }
    ...
    let formattedUrl = pathname;
    if (formattedQuery) {
      formattedUrl += ` - ${formattedQuery.replace(/\n\s+/g, ' ')}`; // Remove quebras de linha e espaços extras
    }
    if (formattedProjection) {
      formattedUrl += ` - ${formattedProjection.replace(/\n\s+/g, ' ')}`; // Remove quebras de linha e espaços extras
    }
...
const router = async function (config) {
  ...
  appRouter.use(logger(config.options.logger));
